...
object OperationsOnPair {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //map
    val tup2Rdd = wordsRdd.map(f=>(f,1))

    //SortByKey
    println("Sort by Key ==>")
    tup2Rdd.sortByKey().foreach(println)

    //reduceByKey
    println("Reduce by Key ==>")
    val wordCount = tup2Rdd.reduceByKey((a,b)=>a+b)
    wordCount.foreach(println)

    def param1= (accu:Int,v:Int) => accu + v
    def param2= (accu1:Int,accu2:Int) => accu1 + accu2

    //AggregateByKey  (same results could be acheived by reducedbykay )
    println("Aggregate by Key ==> wordcount")
    val wordCount2 = tup2Rdd.aggregateByKey(0)(param1,param2)
